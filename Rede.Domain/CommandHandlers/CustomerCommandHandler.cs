...
namespace Rede.Domain.CommandHandlers;
        ...
        IRequestHandler<RemoveCustomerCommand, bool>
    {
        ...
        public Task<bool> Handle(RegisterNewCustomerCommand message, CancellationToken cancellationToken)
        {
            ...
            var customer = new Customer(Guid.NewGuid(), message.Name, message.Email, message.BirthDate, message.Document);
            ...
            if (Commit())
            {
                Bus.RaiseEvent(new CustomerRegisteredEvent(customer.Id, customer.Name, customer.Email, customer.Document, customer.BirthDate));
            }
        ...
        public Task<bool> Handle(UpdateCustomerCommand message, CancellationToken cancellationToken)
        {
            ...
            var customer = new Customer(message.Id, message.Name, message.Email, message.BirthDate, message.Document);
            ...
            if (Commit())
            {
                Bus.RaiseEvent(new CustomerUpdatedEvent(customer.Id, customer.Name, customer.Email, customer.Document, customer.BirthDate));
            }
